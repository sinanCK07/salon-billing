...
export const HistoryList: React.FC = () => {
    const { bills, clearHistory, updateBill, removeBills } = useBillHistory();
    ...
    const filteredBills = React.useMemo(() => {
        if (!searchDate) return bills;
        return bills.filter(b => b.date.startsWith(searchDate));
    }, [bills, searchDate]);

    if (bills.length === 0 && !searchDate) {
        return (
    ...
    return (
        <div className="w-full h-full flex justify-center pb-20 overflow-y-auto">
            ...
            <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 transition-all">
                            {!showDeleteConfirm ? (
                                <button
                                    ...
                                    className="bg-red-600 text-white text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1 hover:bg-red-700 transition-colors shadow-sm"
                                >
                                    <Trash2 size={12} /> Delete Selected ({selectedIds.length})
                                </button>
                            ) : (
                                <div className="flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-full border border-red-100">
                                    <span className="text-red-600 text-[10px] font-bold uppercase whitespace-nowrap">Delete {selectedIds.length} bills?</span>
                                    <button
                                        onClick={() => {
                                            removeBills(selectedIds);
                                            setSelectedIds([]);
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className="bg-green-50 text-green-700 text-xs flex items-center gap-1 hover:bg-green-100 px-3 py-1.5 rounded-full font-bold transition-colors border border-green-100"
                    >
                    ...
                    {!showClearConfirm ? (
                        <button
                            ...
                            className="bg-red-50 text-red-600 text-xs flex items-center gap-1 hover:bg-red-100 px-3 py-1.5 rounded-full font-bold transition-colors border border-red-100"
                        >
                            <Trash2 size={14} /> Clear All
                        </button>
                    ) : (
                        <div className="flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-full border border-red-100">
                            <span className="text-red-600 text-[10px] font-bold uppercase whitespace-nowrap">Sure?</span>
                            <button
                                onClick={() => {
                                    clearHistory();
            ...
            {/* Search Filter */}
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-wrap items-center gap-4 mb-4">
                <div className="flex-1 min-w-[200px]">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Search Bills by Date</label>
                    <input 
                        type="date" 
                        value={searchDate}
                ...
                {searchDate && (
                    <button 
                        ...
                        className="mt-5 text-xs font-bold text-purple-600 hover:text-purple-700 bg-purple-50 px-3 py-2 rounded-lg transition-colors"
                    >
                        Clear Filter
                    </button>
                )}
            ...
            <div className="space-y-3">
                {filteredBills.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                        <FileText size={48} className="mb-2 opacity-50" />
                        <p>{searchDate ? "No bills found for selected date" : "No bills generated yet."}</p>
                    </div>
                ) : (
                    filteredBills.map((bill) => (
                        <div key={bill.id} className={`bg-white p-4 rounded-xl shadow-sm border transition-all flex gap-3 ${selectedIds.includes(bill.id) ? 'border-purple-600 bg-purple-50/30' : 'border-gray-100 hover:border-purple-200'}`}>
                            ...
                            <div className="flex-1">
                                ...
                                {/* Services List */}
                                <div className="text-sm text-gray-600 truncate mb-1 bg-gray-50 px-2 py-1 rounded">
                                    {bill.services.map(s => s.name).join(', ')}
                                </div>

                                {/* Discount Reason */}
                                {bill.discount > 0 && bill.discountReason && (
                                    <div className="text-[10px] text-orange-600 font-bold bg-orange-50 px-2 py-0.5 rounded-full inline-block mt-1 mb-2 border border-orange-100">
                                        DISCOUNT: {bill.discountReason.toUpperCase()}
                                    </div>
                                )}
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-gray-100 mt-2">
                                    ...
                                    <div className="flex gap-1 sm:gap-2">
                                        <button
                                            ...
                                            className="text-green-600 hover:bg-green-50 p-2 rounded-lg transition-colors flex items-center gap-1 text-xs font-bold"
                                        ...
                                        <button
                                            ...
                                            className="text-purple-600 hover:bg-purple-50 p-2 rounded-lg transition-colors flex items-center gap-1 text-xs font-bold"
